...
# TODO need to edit for different camera 
...
cap = cv2.VideoCapture('evgp_footage.avi')
...
while (cap.isOpened()):
    try:
        ...
        steering_angle = compute_steering_angle(frame, lines)
        print(steering_angle)
        lane_lines_image = display_lines(frame, lines)
        final = display_heading_line(lane_lines_image, steering_angle)
        cv2.imshow('Original', frame)
        cv2.imshow('Lines included', final)
        #out.write(lanes_line_image)
...
img = display_lines(frame, lane_lines)
angle = compute_steering_angle(frame, lane_lines)
print(angle)
out = display_heading_line(img, angle)

#cv2.imwrite('lane_lines.png', img)
